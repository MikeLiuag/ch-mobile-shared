...
import {Animated, Easing, AppState, Dimensions, Image, Linking, Platform, ScrollView, StatusBar, StyleSheet, TouchableOpacity} from 'react-native';
import EntypoIcon from 'react-native-vector-icons/Entypo';
import AwesomeIcon from 'react-native-vector-icons/FontAwesome';
import AntIcon from 'react-native-vector-icons/AntDesign';
import {LearningLibraryPlayer} from '../common';
import LottieView from 'lottie-react-native';
import completedAnim from "./../../assets/animations/content-completed";
import {HEADER_NORMAL, HEADER_X,DEFAULT_IMAGE} from '../../constants';
import Overlay from "react-native-modal-overlay";
import LinearGradient from "react-native-linear-gradient";
import {EmptyContent} from "../EmptyContent";
import {addTestID, isIphoneX, isIphone12, toRoman, getHeaderHeight} from "../../utilities";
...
export class EducationalPieceComponent extends Component<props> {
    ...
    getParagraphView = (content, section, i, order, nestedLevel) => {
        ...
        totalContent.content.forEach((content) => {
            ...
            if (content.nodeType === 'hyperlink') {
                ...
                if (marksLength > 0) {
                    ...
                    paragraph.push(
                        <Text key={i + '' + section.length + '' + paragraph.length}
                              style={{...desText, fontSize: 20 + this.state.fontSizeOffset, lineHeight: 30 + this.state.fontSizeOffset}}>{content.value}</Text>
                    );
                }
            } else {
                //For Simple Text
        ...
        return (<View style={{marginLeft: gap}} key={i + '' + section.length}>
            <Text style={{...desText, fontSize: 20 + this.state.fontSizeOffset, lineHeight: 30 + this.state.fontSizeOffset}}>
                {gapOrder}{paragraph}
            </Text>
        </View>);
    };
                ...
                <Container style={styles.container}>
                    <Header style={styles.header}>
                        ...
                        <Left style={styles.headLeft}>
                            ...
                            <Button transparent style={styles.headIcon} onPress={() => {
                                this.showOverlay(false);
                            }}>
                                <AwesomeIcon
                                    {...addTestID('back-icon')}
                                    name="font" size={22} color="#3fb2fe"/>
                            </Button>
                        </Left>
                        <Body/>
                        <Right style={styles.headRight}>
                            <Button
                                {...addTestID('bookmark-content')}
                                transparent style={styles.headIcon} onPress={() => {
                                this.props.bookmarkContent(isBookmarked, this.state.entryId, data.fields.title);
                            }}>
                                <AwesomeIcon name={isBookmarked ? "bookmark" : "bookmark-o"} size={22}
                                             color="red"/>
                            </Button>
                            {this.props.isProviderApp ? (
                                    <Button
                                        {...addTestID('navigate-to-share-content')}
                                        transparent style={styles.headIcon} onPress={() => {
                                        this.props.navigateToShareContent(this.state.data)
                                    }}>
                                        <EntypoIcon name="forward" size={25} color="#3fb2fe"/>
                                    </Button>)
                                : (<Button transparent style={styles.headIcon}
                                           {...addTestID('share-btn')}
                                           onPress={()=>{
                    ...
                    >
                            ...
                            <Image
                                ...
                                source={
                                    data.fields.titleImage
                                        ? {
                                            uri: 'https:' + data.fields.titleImage.fields.file.url,
                                        }
                        ...
                        <View style={styles.singleItem}>
                            ...
                            <View style={styles.authorInfo}>
                                <Text style={styles.authorname}>{data.fields.authorName?data.fields.authorName:null}</Text>
                                <Text style={styles.authorOrganization} numberOfLines={1}>{data.fields.authorOrganization?data.fields.authorOrganization:null}</Text>
                            </View>
                            <View style={styles.contentDurationWrapper}>
                                <Text style={styles.contentDurationText}>{data.fields.contentLengthduration?data.fields.contentLengthduration:null}</Text>
                            </View>
                        </View>
                        ...
                        {!this.props.isProviderApp && data.fields.branchLink && (
                            ...
                            >
                                ...
                                <Text numberofLines={2} style={styles.joinMainText}>Join our weekly virtual group
                                    Loved Ones & Substance Use</Text>
                                <Text numberofLines={1}
                                      style={styles.joinSubText}>Donation based</Text>
                                <Button style={styles.joinCTABtn}
                                        onPress={() => Linking.openURL(data.fields.branchLink)}
                                >
                                    <LinearGradient
                                        start={{ x: 0, y: 0 }}
                                        end={{ x: 0, y: 1 }}
                                        colors={["#fff", "#EFF8FF", "#D8EFFF"]}
                                        style={styles.CTABtnInner}
                                    >
                                        <Text style={styles.joinCTABtnText}>Join Now</Text>
                                    </LinearGradient>
                                </Button>
                            </View>*/}
                            </TouchableOpacity>
                            ...
                            !this.state.isMarkCompleted ?
                                ...
                                <View
                                    ...
                                    <GradientButton
                                        ...
                                        onPress={()=>{this.closeEducationalContentAnim(data.fields.title)}}
                        ...
                        this.getContentAudioUri() && (
                            <Animated.View
                                style={{ backgroundColor:'transparent',
                                    marginTop: -10,
                                    borderTopLeftRadius: 16, borderTopRightRadius: 16, overflow: 'hidden', height: this.state.x }}>
                                        ...
                                        this.state.slideAudio && (
                                            <View>
                                                ...
                                                <View style={styles.palyerText}>
                                                    <Text style={styles.playerMainText} numberOfLines={3}>{data.fields.title}</Text>
                                                    <Text style={styles.playerSubText}>{this.state.activeArticleIndex+1} of {(this.props.educationOrder && this.props.educationOrder.length>0)?this.props.educationOrder.length:1}</Text>
                                                </View>
                                            </View>
                                        )
                                    }
    ...
    closeEducationalContentAnim = async (entryTitle) => {
        this.setState({showAnimation: true});
        this.props.markContentAsComplete(this.state.entryId, entryTitle);
        setTimeout(() => {
            // this.setState({showAnimation: false});
            this.props.captureFeedback(this.state.entryId);
        }, 2000);
    };
}
